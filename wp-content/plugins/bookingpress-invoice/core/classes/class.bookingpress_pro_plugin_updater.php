...
class bookingpress_pro_updater {
	...
	public function show_update_notification( $file, $plugin ) {
		...
		printf(
			...
			esc_html__( 'There is a new version of %1$s available.', 'bookingpress-invoice' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Contact your network administrator to install the update.', 'bookingpress-invoice' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
				/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				__( '%1$sView version %2$s details%3$s.', 'bookingpress-invoice' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped --Reason: data is escaped properly
			...
			printf(
				...
				__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'bookingpress-invoice' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped --Reason: data is escaped properly
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Update now.', 'bookingpress-invoice' ),
				'</a>'
			);
		}
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'bookingpress-invoice' ), esc_html__( 'Error', 'bookingpress-invoice' ), array( 'response' => 403 ) );
		}
